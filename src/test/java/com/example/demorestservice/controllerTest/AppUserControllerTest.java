...
public class AppUserControllerTest {
    ...
    void should_be_able_to_login() throws Exception {
        ...
        User user = new User("test", "12345", new ArrayList<>());

        when(userDetailsService.loadUserByUsername(loginRequestDto.getUsername())).thenReturn(user);
        try( MockedStatic<FilterUtil> util = Mockito.mockStatic(FilterUtil.class)){
            util.when(() -> FilterUtil.generateToken(request, user)).thenReturn("new_token");
        }
